(function(_window) {
	...
	if (typeof require === 'function' && typeof exports === 'object' && typeof module === 'object') {
		...
		//Niman
		Niman.define('nvm', [], function(context) {
			context.npromise = Nvm;
		});
	} else {
		//window
		window.NimanViewModel = Nvm;
	}
})(this);
...
define(function(require, exports, module) {
	...
	function Nvm(element) {
		...
		this._change = function(evt) {
			...
			if (src._dataPath) {
				var path = Nvm.formatBindPath(bindMap.value);
				path = path.replace(src._dataPath, '');
				self.setValue(src._dataPath, path, src.value);
			}
		};
	};
